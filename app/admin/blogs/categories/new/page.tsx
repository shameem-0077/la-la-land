...
export default function BlogCategoryFormPage() {
  ...
  const uploadToStorage = async (file: File) => {
    ...
    const fileName = `blog_cat_${Math.random()}.${fileExt}`;
    const filePath = `blog-categories/${fileName}`;

    const { error: uploadError } = await supabase.storage
      .from('blog-media') // Updated bucket
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = isEdit 
        ? await supabase.from("BlogCategory").update(formData).eq("id", params.id)
        : await supabase.from("BlogCategory").insert([formData]);
    ...
    <div className="max-w-3xl mx-auto space-y-8 pb-20">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-6">
          <Link href="/admin/blogs/categories" className="w-12 h-12 rounded-2xl bg-white border border-slate-200 flex items-center justify-center text-slate-400 hover:text-blue-600 transition-all shadow-sm">
